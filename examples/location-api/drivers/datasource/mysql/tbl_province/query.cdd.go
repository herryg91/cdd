...
func (r *MysqlDatasource) GetByPrimaryKey(id int) (*ProvinceModel, error) {
	result := &ProvinceModel{}
	err := r.db.Table(r.tableName).Where("id = ?", id).Scan(&result).Error
	return result, err
}

func (r *MysqlDatasource) GetAll() ([]*ProvinceModel, error) {
	result := []*ProvinceModel{}
	err := r.db.Table(r.tableName).Find(&result).Error
	return result, err
}

func (r *MysqlDatasource) Create(in ProvinceModel) (*ProvinceModel, error) {
	timeNow := time.Now()
...
func (r *MysqlDatasource) Update(in ProvinceModel) (*ProvinceModel, error) {
	timeNow := time.Now()
	in.CreatedAt = nil
	in.UpdatedAt = &timeNow
...
func (r *MysqlDatasource) Delete(id int) error {
	return r.db.Table(r.tableName).Delete(&ProvinceModel{}, "id = ?", id).Error
}
